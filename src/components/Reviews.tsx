...
function ReviewGrid() {
    ...
    return (<div ref={containerRef} className="relative -mx-4 mt-16 grid
    h-[49rem] max-h-[150rem] grid-cols-1 items-start gap-8 overflow-hidden
    px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3">
        ...
        bg-gradient-to-b from-slate-100"></div>
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32
        bg-gradient-to-t from-slate-100"></div>
    </div>)
}
...
function Review({imgSrc, className, ...props}: ReviewProps) {
    ...
    const POSSIBLE_ANIMATIONS_DELAY = [
        "0s",
        "0.1s",
        "0.2s",
        "0.3s",
    ...
    const animationDelay = POSSIBLE_ANIMATIONS_DELAY[
